...
PREPARE=1 #flag for PrePARE check

# options controller 
#OPTIND=1
while getopts "h:e:y:a:o:p:c:r:v:u:" OPT; do
    case "$OPT" in
...
OPT_ATM="$BASE_OPT,ATM=1,OCE=0,VEG=1,NCORESATM=$NCORESATM"
...
if [[ "$SUBMIT" == "qsub" ]] ; then
        ...
        JOB_OCE='$SUBMIT ${MACHINE_OPT} -v ${OPT_OCE} -l EC_threads_per_task=$NCORESOCE -N nemo-${expname}-${year}
                 ...
                 ./call_ece2cmor3.sh'
	JOB_VEG='$SUBMIT ${MACHINE_OPT} -v ${OPT_VEG} -l EC_threads_per_task=$NCORESOCE -N lpjg-${expname}-${year}
