...
# options controller 
OPTIND=1
while getopts ":h:l:y:u:a:o" OPT; do
    case "$OPT" in
...
function runece2cmor_oce {
    ...
    echo "  Using $PREFIX tables"
    echo "================================================================" 
    $ece2cmor $OCEDIR $YEAR-01-01 --exp $EXP --conf $CONFIGFILE --vars $VARLIST --npp $THREADS --tmpdir $TMPDIR --nemopar $NEMOPAR --tabid $PREFIX --tabdir $TABDIR --mode append --oce    
...
if [ "$ATM" -eq 1 ]; then
    ...
    wait
    for MON in $MONTHS ; do
    	if [[ ${DO_PRIMA} == true ]] ; then 
	     runece2cmor_atm PRIMAVERA $NCORESATM $YEAR $MON & 
	fi
    done
    wait
fi
...
if [ "$OCE" -eq 1 ]; then
    ...
    for YEAR in $YEARS ; do
    	if [[ ${DO_PRIMA} == true ]] ; then 
		runece2cmor_oce PRIMAVERA $NCORESOCE $YEAR & 
 	fi
    done
    wait
fi
